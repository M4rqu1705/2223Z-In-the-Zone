...
enum driveDirectionEnum {Forward = 0, Backward, TurnLeft, TurnRight };   //Enum to indicate which directions can the drive translate
typedef struct{
	joystickToggleButtonStruct directionNormal;   //Struct to toggle change direction button based on the joystickToggleButtonStruct
	...
	signed byte joystickInputs[2];                //Array to store the joystick analog inputs. 0 is forward input, 1 is side input. Used to not call for the current joystick values various times
...
void driveOperatorControl(bool test){
	if(test){
		...
		//Move motors using calculated values for left and right side
		motor[MOTOR_driveLF] = motor[MOTOR_driveLB] = motor[MOTOR_driveLM] = drive.outputs[0];
		motor[MOTOR_driveRF] = motor[MOTOR_driveRB] = motor[MOTOR_driveRM] = drive.outputs[1];
		//motor[MOTOR_driveLM] = (drive.outputs[0]*0.75);
		//motor[MOTOR_driveRM] = (drive.outputs[1]*0.75);
	}
}
