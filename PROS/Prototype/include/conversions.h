...
#define WITHIN_RANGE(inNumber) ((abs(inNumber) > 127) ? ((inNumber) < -127) ? -127 : 127 : 0)
#define INCHES_TRANSLATION_TO_ENCODER_PULSES(inches) (((WHEEL_DIAMETER)*3.1415926535897932384626433832795)*((inches) / 360))
#define DEGREES_ROTATION_TO_GYRO_TICKS(degrees) (ROUND((degrees) *((GYRO_FULL_ROTATION_TICKS) / 360)))
#define DEGREES_ROTATION_TO_ENCODER_PULSES(degrees) (ROUND((((degrees) / 360) * (DRIVE_WIDTH) * 3.1415926535897932384626433832795) / ((WHEEL_DIAMETER) / 360)))
